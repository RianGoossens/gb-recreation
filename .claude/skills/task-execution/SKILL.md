...
0b. Then check GitHub issues authored by Rian (see the github-issues skill). Hard-check the author: only Rian's own issues count. An issue needs attention when it is open and does not have the `awaiting-review` label (fresh, or handed back by Rian removing the label). If one does, handle the oldest this run instead of a plan task: do the work, comment what you did, add the `awaiting-review` label, and stop. Never close issues yourself; Rian closes them when satisfied. Issues rank above the plan and below the inbox.
