...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [ ] Stitch the full scrolling width: a first attempt (dead reckoning plus periodic 32-wide ring-buffer reads) confirmed the buffer preloads about 7 columns beyond the visible screen but no further, and confirmed a naive "hold right forever, never jump" script dies and respawns partway through (world column ~26-30, a pit), silently corrupting dead-reckoned position past that point (docs/reference/level-1-1.md). Needs a script that actually survives hazards, or spawn-reset detection to discard corrupted runs, before recording tilemap and scroll per screen for real
