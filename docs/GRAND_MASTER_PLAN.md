...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Read the level out of the ROM instead of playing through it (Rian, issue 5). The geometry is in the cartridge: `tools/find_level_data.py` ruled out every direct tilemap form, a density scan found the data in bank 2, and `tools/decode_level.py` decodes the format (`0xFE` starts a column, then `(row<<4|count)` plus that many tile bytes). World 1-1's records start at 0x0A2BD and the first 20 columns decode to exactly what the emulator renders. It desynchronises from column 20, so one control code is still unaccounted for; finding it is worth more than any further work on the walker (docs/reference/level-1-1.md)
