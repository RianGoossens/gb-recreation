...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Classify which tile IDs are solid by observing Mario's actual collisions against each one while walking through
      ...
      - [~] Pin the step/pyramid structure's solid tiles precisely: exact per-column tile grid now mapped from the static spawn tilemap (docs/reference/level-1-1.md), but solidity is still not confirmed by direct collision. The "release Right, then jump" trick that worked for the world-column-48 hazard doesn't transfer here (Right stays held through the whole flight, so speed ramps back up mid-air regardless of takeoff speed); still needs a controlled fall from directly above a column, or Right released for the whole flight, not just before it. Presumed solid by level-design consistency in the meantime
    - [~] Stitch the full scrolling width: survival is solved (jumping at anything nearby instead of just pausing let `tools/stitch_level_1_1.py` travel past world column 1880 without dying). Position-based lap-numbering for the tile tracker went through two rounds of bugs found by actually running the tool and sanity-checking against screenshots, not just reasoning about the code: a negative-column edge case (fixed), then a bigger one, `0xC20C` does not hold horizontal speed while airborne at all (it climbs unboundedly during a jump), so the position estimate was badly inflated by this tool's own frequent stomp-jumps. Fixed by only trusting it while grounded. A repeat-inference addition (a slot with no detected change in over a lap is assumed to hold repeated terrain, not stuck) plus this fix now gives a 626-wide map for a 5000-frame run, but whether that distance is itself accurate hasn't been independently cross-checked, stated plainly as open rather than assumed (docs/reference/level-1-1.md)
    - [ ] Convert the confirmed grid into `Level`/`Solids` and wire it in, ROM-gated, once stitching covers enough of the level to be worth it
