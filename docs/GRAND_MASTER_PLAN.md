...
- [ ] Replace the ROM with a hash-verified dump; add a `verify-rom` command that checks SHA-1, MD5, CRC32 and refuses to proceed on mismatch
- [ ] Study the title screen logic and tile data via the `10yard/supermarioland` reference; write down the memory map notes in `docs/reference/`
...
- [ ] Headless screenshot command that renders a given state to PNG
