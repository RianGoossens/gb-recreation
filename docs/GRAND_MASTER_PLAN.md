...
- [~] Remaining World 1 levels (the original's real geometry)
  ...
  - [ ] **Pin where 1-2 and 1-3 actually start, by capturing them from the running game.** All three lists open with a three-pointer shape and 1-1's verified start is six bytes into the first one, which suggests 1-2 starts at 0x0A1BD and 1-3 at 0x0A1E0. That is a guess and is deliberately not shipped: the one list start we know was pinned by playing it, and these have not been. The blocker is reaching 1-2 in the emulator. `run_through_levels.py` gets through 1-1 and captures the "WORLD 1-2" title card, then stops producing columns, because the pinned Y is chosen for 1-1's geometry
