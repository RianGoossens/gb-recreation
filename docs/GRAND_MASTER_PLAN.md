...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Classify which tile IDs are solid by observing Mario's actual collisions against each one while walking through
      ...
      - [~] Pin the step/pyramid structure's solid tiles precisely: exact per-column tile grid now mapped from the static spawn tilemap (docs/reference/level-1-1.md), but solidity is still not confirmed by direct collision, every running jump tried clears the whole structure without landing on it. Needs a slower approach or a controlled fall from above; presumed solid by level-design consistency in the meantime
    - [ ] Find a reliable per-frame read of the real scroll amount (naive once-per-frame SCX is aliased by the status-bar split; `0xC20B` was checked and ruled out, it is an input-hold timer, not scroll, see the doc). Dead reckoning from Mario's known walk-speed curve worked for the pyramid grid within the initially loaded 20x18 screen; a hook-based SCX read or extending dead reckoning is the likely path beyond it
    - [ ] Stitch the full scrolling width: a first attempt (dead reckoning plus periodic 32-wide ring-buffer reads) confirmed the buffer preloads about 7 columns beyond the visible screen but no further, and confirmed a naive "hold right forever, never jump, never react" script dies and respawns around world column 48, most likely from walking into an enemy (Y position never changes through the reset, and an OAM sprite sits inside Mario's own hitbox the frame before, so it isn't a pit), silently corrupting dead-reckoned position past that point (docs/reference/level-1-1.md). Needs a script that actually survives hazards, or spawn-reset detection to discard corrupted runs, before recording tilemap and scroll per screen for real
