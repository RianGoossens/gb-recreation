...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [x] Classify which tile IDs are solid by observing Mario's actual collisions against each one while walking through
    ...
    - [~] Read the level out of the ROM instead of playing through it (Rian, issue 5). The geometry is in the cartridge: `tools/find_level_data.py` ruled out every direct tilemap form, a density scan found the data in bank 2, and `tools/decode_level.py` decodes the format (`0xFE` starts a column, then `(row<<4|count)` plus that many tile bytes). World 1-1's streaming records start at 0x0A206 and 130 of them decode to world columns 21-150, matching the running game on 66 of the 67 columns there is ground truth for. Columns 0-20 (one screen) are loaded separately and are the last piece to find; this is worth more than any further work on the walker (docs/reference/level-1-1.md)
    - [~] Stitch the full scrolling width. Superseded once the scroll was actually measured (see the subtask above): the old "travelled past world column 1880" and its successors were artifacts of a broken position estimate, and the walker was in fact stuck against a pipe at world column 17. The real figure is world column 78, where a flying enemy on a pillar at the edge of a gap ends every run. `tools/stitch_level_1_1.py` now checkpoints the emulator, the scroll tracker and the map together, rewinds on death and reseeds the walker, and requires real scroll progress per segment so standing still cannot count as survival. That search explores genuinely different routes and every one converges on the same spot, so the leading hypothesis is that the route continues below through the gap rather than that it is a timing problem. Output is a bounded 96-column map (docs/reference/level-1-1.md)
    - [~] Convert the confirmed grid into `Level`/`Solids` and wire it in, ROM-gated: a first version now exists for the opening screen only (`tools/convert_level_1_1_to_level_format.py` writes a plain-text level our own `Level::from_file` already loads, gitignored and regenerated on demand, not committed). Extending this to the full stitched width waits on that data being trustworthy first (see the stitching subtask above)
