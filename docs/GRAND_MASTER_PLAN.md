...
- [~] Remaining World 1 levels (the original's real geometry)
  ...
  - [ ] Pin World 1-3's start the same way (candidate 0x0A1E0). Needs the walkthrough to resume through 1-2, which it does not do yet
  - [~] Capture World 1-2's columns from the running game. `run_through_levels.py` crosses level boundaries now and gets 69 of 1-2's 280 columns, all matching the ROM, re-verified six times over as Mario dies at world column 69 and the level restarts. Three fixes were needed: tap only A through the bonus game (Start pauses Super Mario Land, and a tap landing as the next level opens leaves it paused, which looks exactly like a movement bug), pin Mario's Y but not the rise/fall phase byte (pinning the phase freezes him at any spawn he did not start at), and raise the end-of-level threshold from 90 quiet frames to 400 (1-2 goes quiet mid-level). Remaining: find what kills him at column 69 and get past it (docs/reference/level-1-1.md)
  - [ ] Wire the verified levels into a campaign once their starts are pinned
