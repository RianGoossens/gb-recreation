...
- [~] Decode the cartridge's object table so the extracted levels carry their real enemies
  ...
  - [ ] Work out which kind byte is which named enemy. `tools/capture_object_sprites.py` draws each kind's sprite off the running game, which separates them but does not name them, and a wrong guess would put an invented enemy in through the back door
