...
- [~] Decode the cartridge's object table so the extracted levels carry their real enemies
  ...
  - [ ] Work out which kind byte is which enemy. Five kinds spawn in World 1-1: 0x00 (ten times), 0x0E (three), 0x04, 0x0A, 0x0B (once each). `tools/capture_object_sprites.py` draws each kind's sprite off the running game, which is enough to tell them apart but not yet to name them
