...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Stitch the full scrolling width: survival is solved (jumping at anything nearby instead of just pausing let `tools/stitch_level_1_1.py` travel past world column 1880 without dying, versus dying around 48-77 with earlier reactions), but the tile-tracking method itself now has a known blind spot at that range: it only detects a slot's new lap when the tile *value* changes, so long uniform terrain (a flat stretch of repeated ground tiles) produces no detected change and the tracked map silently stalls (confirmed: a 630-column real run only yielded a 64-wide tracked map). Needs a tracking method that does not depend on content changing before this can go further (docs/reference/level-1-1.md)
