...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Stitch the full scrolling width: `tools/stitch_level_1_1.py` does this correctly and without guessing, tracking each of the 32 ring-buffer columns frame by frame and incrementing its world-column identity by 32 every time its content actually changes (no scroll register or position estimate needed for correctness). A jump-timing sweep couldn't clear the first hazard (a moving enemy around world column 48) at full running speed, but a "walk, then briefly release, repeat" rhythm survives it (no jump needed at all) and pushed confirmed coverage to world column 64. Still a heuristic (delays the next death, doesn't fix it); an OAM-based enemy-reaction script would go further (docs/reference/level-1-1.md)
