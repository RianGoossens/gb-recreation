...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [ ] Find a reliable per-frame read of the real scroll amount (naive once-per-frame SCX is aliased by the status-bar split; `0xC20B` was checked and ruled out, it is an input-hold timer, not scroll, see the doc). Dead reckoning from Mario's known walk-speed curve worked for the pyramid grid within the initially loaded 20x18 screen; a hook-based SCX read or extending dead reckoning is the likely path beyond it. Then stitch the full scrolling width by walking through the whole level, recording tilemap and scroll per screen
