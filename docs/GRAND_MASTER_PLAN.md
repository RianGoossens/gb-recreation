...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Stitch the full scrolling width: survival is solved (jumping at anything nearby instead of just pausing let `tools/stitch_level_1_1.py` travel past world column 1880 without dying). Position-based lap-numbering for the tile tracker went through two rounds of bugs found by actually running the tool and sanity-checking against screenshots, not just reasoning about the code: a negative-column edge case (fixed), then a bigger one, `0xC20C` does not hold horizontal speed while airborne at all, so the position estimate was badly inflated by this tool's own frequent stomp-jumps. Fixed by only trusting it while grounded. Cross-checked the fix by replaying the exact recorded button sequence through our own engine: the grounded-only case matches expected physics exactly (confirmed over a 700-frame jump-free stretch); the airborne-freeze assumption across many jumps couldn't be cleanly tested the same way because our own engine didn't reproduce the cartridge's jump timing at the time, a real confound, not evidence either way. Retried after the engine gained the measured three-regime jump model: a fresh 500-frame recorded run puts the PyBoy position estimate at column 58 versus the replayed engine's column 67, a 14.7% gap, down from the original 17% but still real (the implemented jump takes about 26 frames per jump versus the cartridge's about 24, and this run spent 170 of its 500 frames airborne). Narrower, still not settled (docs/reference/level-1-1.md)
