...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Stitch the full scrolling width: `tools/stitch_level_1_1.py` now does this correctly and without guessing, tracking each of the 32 ring-buffer columns frame by frame and incrementing its world-column identity by 32 every time its content actually changes (no scroll register or position estimate needed for correctness). Confirmed out to world column 63 so far, limited only by a "hold Right, never jump, never react" run dying to an enemy around world column 48 (docs/reference/level-1-1.md). Needs a run that survives past that hazard to go further
