...
- [~] Level data model and 1-1 tilemap extracted from the reference/ROM
  ...
  - [~] Extract the scrolling World 1-1 tilemap from the ROM/emulator into level data
    ...
    - [~] Stitch the full scrolling width: survival is solved (jumping at anything nearby instead of just pausing let `tools/stitch_level_1_1.py` travel past world column 1880 without dying). The mislabeling risk in the tile-tracking is also fixed: a slot's world column is now computed from a precise per-frame position estimate (0xC20C integrated every frame) when a change is detected, not a blind "+32 from last label", and a follow-up fix rejects negative/regressing candidates a naive nearest-lap pick could otherwise choose near spawn. A 5000-frame run now yields a clean 141-wide map with 45 columns actually confirmed (0-26 solid ground, a scattered handful further out). Genuinely uniform terrain (nothing ever changes) still can't be detected by this method at all, by nature; what's left is coverage, not correctness (docs/reference/level-1-1.md)
