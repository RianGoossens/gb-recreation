...
| Drop block (kind `0x36`) | measured, **correctly absent** | The most-used kind in the game, 50 records across nine levels. It never moves in 600 frames, never costs Mario a life at any overlap, and goes away the frame his feet reach its surface, which no other contact does and which it never does on its own (`tools/probe_drop_block.py`). A block that gives way when stood on. Left out of the levels because what happens in that one frame, whether it holds him for it and whether the slot emptying is a removal or a hand-off, is not settled. |
...
| Which of Mario's frames plays when | canonical | traced on the running game (`tools/trace_mario_frames.py`). Standing is the first block, a walk cycles through three blocks including that one at about four frames each, and airborne is a single jump pose (block 4) that does not change between rising and falling. Unmeasured: whether the walk's rate changes with his speed |
...
| An enemy's body | **stand-in** | 8 by 8 for every kind, which the engine has always used and nothing has measured. It is what walks into walls and stands on floors, a separate thing from the contact box above. The drawings are bigger than it, so a sprite is anchored to the body's bottom left corner and overhangs it |
